    ...
    form.addEventListener('submit', async function(e){
        ...
        for (const h of hotels) {
            ...
            <div class="card h-100">
                ${h.image_path ? `<img src="${assetBase}${escapeHtml(h.image_path)}" class="card-img-top" alt="${escapeHtml(h.name)}">` : ''}
                <div class="card-body">
                    <h5 class="card-title">${escapeHtml(h.name)}</h5>
                    <p class="card-text">${escapeHtml(h.location)} — ${Number(h.price).toFixed(2)} €</p>
