		...
		<div class="row">
			<div class="col-md-8 offset-md-2">
					...
					<ul class="list-group mb-3">
						<?php foreach ($rooms as $r): ?>
							...
							<li class="list-group-item d-flex justify-content-between align-items-center">
								<span>Room #<?= htmlspecialchars($room->getId()) ?> — Beds: <?= htmlspecialchars($room->getBeds()) ?></span>
								<span>
									<a class="btn btn-sm btn-danger"
								   data-bs-toggle="modal" data-bs-target="#confirm-modal"
								   data-href="<?= $link->url('hotel.deleteRoom') ?>&id=<?= urlencode($room->getId()) ?>"
								   data-message="Delete Room #<?= htmlspecialchars($room->getId()) ?> (<?= htmlspecialchars($room->getBeds()) ?> beds)?">Delete</a>
								</span>
							</li>
						<?php endforeach; ?>
					</ul>
				...
				<form method="post" action="<?= $link->url('hotel.addRoom') ?>">
					<input type="hidden" name="hotel_id" value="<?= htmlspecialchars($hotel->getId()) ?>">
